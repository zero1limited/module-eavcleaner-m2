...
class StoreValueToDefaultCommand extends Command
{
    /** @var IteratorFactory */
    ...
    {
        $description = "Set global value for product to that of the value at store view.";
        $this
            ->setName('eav:attributes:store-value-to-default')
    ...
    public function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if(!$storeId){
            $output->writeln('You must specify a store ID');
            return 1;
        }

        if(!$attributeId){
            $output->writeln('You must specify an attribute ID');
            return 1;
        }
        ...
        foreach ($tables as $table) {
            ...
            $iterator->walk($query, [function (array $result) use ($column, &$counts, $dbRead, $dbWrite, $fullTableName, $isDryRun, $output): void {
                $row = $result['row'];
                $output->writeln('updating entity '.$row['entity_id'].' value: '.$row['value']);
                if(!$isDryRun){
                    $dbWrite->query(
                        'INSERT INTO '.$fullTableName.' ('.$column.', attribute_id, store_id, value) values (?, ?, ?, ?) ON DUPLICATE KEY UPDATE value = ?',
                        [
                            $row['entity_id'],
                            $row['attribute_id'],
                            0,
